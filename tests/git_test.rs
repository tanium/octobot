...
fn test_has_branch() {
    ...
    assert_eq!(true, git.git.has_branch("master").unwrap(), "should have master branch");

    git.run_git(&["branch", "falcon"]);
    git.run_git(&["branch", "eagle"]);

    assert_eq!(true, git.git.has_branch("falcon").unwrap(), "should have falcon branch");
    assert_eq!(true, git.git.has_branch("eagle").unwrap(), "should have eagle branch");
    assert_eq!(false, git.git.has_branch("eagles").unwrap(), "should NOT have eagles branch");
}
...
fn test_does_branch_contain() {
    ...
    assert_eq!(true, git.git.does_branch_contain(&commit, "master").unwrap(), "master should contain commit");
    assert_eq!(true, git.git.does_branch_contain(&commit, "eagle").unwrap(), "eagle should contain commit");
    assert_eq!(true, git.git.does_branch_contain(&commit, "falcon").unwrap(), "falcon should contain commit");

    git.run_git(&["checkout", "-b", "stallion"]);
    git.add_repo_file("horses.txt", "Stallion\nSteed\nMustang\n", "Horses and stuff");

    let commit2 = git.run_git(&["rev-parse", "HEAD"]);
    assert_eq!(true, git.git.does_branch_contain(&commit, "stallion").unwrap(), "stallion should contain commit");
    assert_eq!(true, git.git.does_branch_contain(&commit2, "stallion").unwrap(), "stallion should contain commit2");

    assert_eq!(false, git.git.does_branch_contain(&commit2, "master").unwrap(), "master should not contain commit2");
    assert_eq!(false, git.git.does_branch_contain(&commit2, "eagle").unwrap(), "eagle should not contain commit2");
    assert_eq!(false, git.git.does_branch_contain(&commit2, "falcon").unwrap(), "falcon should not contain commit2");
}
...
fn test_find_base_commit() {
    ...
    assert_eq!(base_commit.clone(), git.git.find_base_branch_commit(&falcon_commit, "master").unwrap());
    assert_eq!(base_commit.clone(), git.git.find_base_branch_commit("origin/falcon", "master").unwrap());

    // single commit back
    assert_eq!(base_commit.clone(), git.git.find_base_branch_commit(&horses_commit, "master").unwrap());
    assert_eq!(base_commit.clone(), git.git.find_base_branch_commit("origin/horses", "master").unwrap());
    ...
    assert_eq!(base_commit.clone(), git.git.find_base_branch_commit(&falcon_commit, "master").unwrap());
    assert_eq!(new_base_commit.clone(), git.git.find_base_branch_commit(&new_falcon_commit, "master").unwrap());
    assert_eq!(new_base_commit.clone(), git.git.find_base_branch_commit("falcon", "master").unwrap());
    ...
    git.run_git(&["commit", "--amend", "-m", "some other commit 2 -- muaha. re-written!"]);
    git.run_git(&["push", "-f"]);
    ...
    git.reclone();
    assert_eq!(some_commit_1.clone(), git.git.find_base_branch_commit(&new_falcon_commit, "master").unwrap());
}
...
fn test_checkout_branch_new_local_branch() {
    ...
    assert_eq!((), git.git.checkout_branch("some-new-branch", "origin/master").unwrap());
...
fn test_checkout_branch_with_ref() {
    ...
    assert_eq!((), git.git.checkout_branch("some-new-branch", &base_commit).unwrap());
    let now_commit = git.run_git(&["rev-parse", "HEAD"]);
...
fn test_checkout_branch_already_checked_out() {
    ...
    assert_eq!((), git.git.checkout_branch("master", "origin/master").unwrap());
    let now_commit = git.run_git(&["rev-parse", "HEAD"]);
...
fn test_checkout_branch_already_exists() {
    ...
    assert_eq!((), git.git.checkout_branch("the-branch", "origin/master").unwrap());
    let now_commit = git.run_git(&["rev-parse", "HEAD"]);
...
fn test_get_commit_desc() {
    ...
    assert_eq!(("I have just a subject".into(), String::new()), git.git.get_commit_desc("HEAD").unwrap());
